...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>لوحة تحكم البوت المتقدم</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .file-upload p { color: #666; margin-bottom: 10px; }
        .file-preview {
            margin-top: 15px;
            max-width: 100%;
            border-radius: 8px;
...
class DiscordWebBot:
    ...
    def get_guild_channels(self, guild_id, force_refresh=False):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.channels_cache[guild_id] = {'channels': channels, 'timestamp': current_time}
    ...
    def create_embed(self, title=None, description=None, color="#5865F2", image_url=None):
        embed = {"color": int(color.replace("#", ""), 16), "type": "rich"}
        if title:
            embed["title"] = title
        if description:
...
def api_send_message():
    ...
    try:
        ...
        if image_file:
            ...
            success, uploaded_url = discord_bot.upload_image_to_discord(channel_id, image_file.read(), image_file.filename)
            if success:
        ...
        success, result = discord_bot.send_message(channel_id, message if not embed else '', embed=embed)
