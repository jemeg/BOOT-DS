...
class InteractiveRulesBot:
    ...
    def send_interactive_rules(self, channel_id, member_name=None, image_url=None):
        """إرسال رسالة القوانين التفاعلية"""
        # استخدام القوانين الحالية من الملف
        rules = load_rules()
        
        # إنشاء أزرار تفاعلية لكل فئة
        components = []
        ...
        embed = {
            ...
            "description": f"مرحباً بك {member_name or 'في السيرفر'}! 🎊\n\nاختر فئة القوانين التي تريد مشاهدتها:",
            "color": 0x667eea,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ تم إرسال رسالة القوانين التفاعلية بنجاح")
                return True, result
            else:
                logger.error(f"❌ خطأ في إرسال رسالة القوانين التفاعلية: {response.status_code}")
                return False, response.text
        except Exception as e:
            logger.error(f"❌ استثناء في إرسال رسالة القوانين التفاعلية: {e}")
            return False, str(e)
    
    def send_rules_response(self, channel_id, category, member_name=None, message_id=None):
        """إرسال رد بقوانين الفئة المحددة"""
        # استخدام القوانين الحالية من الملف
        rules = load_rules().get(category, [])
        if not rules:
            return False, "فئة غير موجودة"
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                logger.info(f"✅ تم إرسال قوانين {category} بنجاح")
                return True, result
            else:
                logger.error(f"❌ خطأ في إرسال قوانين {category}: {response.status_code}")
                return False, response.text
        except Exception as e:
            logger.error(f"❌ استثناء في إرسال قوانين {category}: {e}")
            return False, str(e)
...
class InteractiveWebSocket:
    ...
    def send_identify(self):
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "os": "linux",
                    "browser": "interactive_rules_bot",
                    "device": "interactive_rules_bot"
                }
            }
        }
    ...
    def handle_interaction(self, interaction):
        ...
        try:
            ...
            if interaction_type == 3:  # Button click
                ...
                # استخراج اسم الفئة من custom_id
                if custom_id.startswith('rules_'):
                    category = custom_id.replace('rules_', '')
                    
                    # إرسال رد بقوانين الفئة
                    success, result = self.bot.send_rules_response(
                        channel_id, 
                        category, 
                        username
                    )
                    ...
                    if success:
                        # إرسال استجابة للتفاعل
                        self.send_interaction_response(interaction, {
                            "type": 4,  # Channel Message with Source
                            "data": {
                                "content": f"تم إرسال قوانين {category} بنجاح!",
                                "flags": 64  # Ephemeral (visible only to the user who clicked)
                            }
                        })
                        logger.info(f"🎯 تم الرد على تفاعل {category} للمستخدم {username}")
                    else:
                        logger.error(f"❌ فشل في الرد على تفاعل {category}")
                        
        except Exception as e:
            logger.error(f"❌ خطأ في معالجة التفاعل: {e}")
    
    def send_interaction_response(self, interaction, response_data):
        """إرسال استجابة للتفاعل"""
        url = f"{self.bot.base_url}/interactions/{interaction['id']}/{interaction['token']}/callback"
        
        try:
            response = requests.post(url, headers=self.bot.headers, json=response_data, timeout=10)
            if response.status_code == 204:
                logger.info("✅ تم إرسال استجابة التفاعل بنجاح")
                return True
            else:
                logger.error(f"❌ خطأ في إرسال استجابة التفاعل: {response.status_code}")
                return False
        except Exception as e:
            logger.error(f"❌ استثناء في إرسال استجابة التفاعل: {e}")
            return False

# إنشاء كائن البوت
bot = InteractiveRulesBot()
interactive_websocket = InteractiveWebSocket(bot)
    ...
    <div class="container">
        ...
        <script>
            ...
            function sendInteractiveRules() {
                ...
                const imageFile = document.getElementById('rulesImage').files[0];
...
def dashboard():
    ...
    for category, rules in current_rules.items():
...
def api_send_interactive_rules():
    ...
    try:
        ...
        if image_file and image_file.filename:
            ...
            # حالياً سنستخدم صورة افتراضية
            image_url = "https://via.placeholder.com/400x200/667eea/ffffff?text=Interactive+Rules"
        
        success, result = bot.send_interactive_rules(channel_id, member_name, image_url)
...
def api_websocket_status():
    """فحص حالة WebSocket"""
    return jsonify({
        'connected': interactive_websocket.running,
        'timestamp': time.time()
    })
...
def test():
    return jsonify({
        ...
        "websocket_connected": interactive_websocket.running,
        "rules_count": len(DEFAULT_RULES),
        "total_rules": sum(len(rules) for rules in DEFAULT_RULES.values())
    })
...
if __name__ == "__main__":
    ...
    if connected:
        logger.info("✅ بوت القوانين التفاعلي متصل وجاهز للعمل")
        
        # بدء WebSocket للمراقبة
        try:
            if interactive_websocket.connect():
                logger.info("✅ WebSocket للقوانين التفاعلية بدأ العمل")
        except Exception as e:
