...
class AdvancedBot:
    ...
    def upload_image(self, image_data):
        ...
        url = f"{self.base_url}/channels/{os.getenv('UPLOAD_CHANNEL_ID', '0')}/messages"
        
        files = {
            'file1': (image_data.filename, image_data.stream, image_data.content_type)
        }
...
<head>
    ...
    <title>تسجيل الدخول - لوحة تحكم القوانين المتقدمة</title>
    <style>
...
<body>
    <div class="login-container">
        <h1>📜 لوحة تحكم القوانين المتقدمة</h1>
        ...
        <div class="bot-info"><p>لوحة تحكم بوت القوانين المتقدم</p></div>
    </div>
</body>
</html>
"""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>لوحة تحكم القوانين المتقدمة</title>
...
<body>
    ...
    <script>
        ...
        function sendRules() {
            ...
            formData.append('rules_title', selectedCategory);
            formData.append('rules_list', JSON.stringify(selectedRules));
            
            if (imageFile) {
                formData.append('image', imageFile);
            }
            
            fetch('/api/send-advanced-rules', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
...
@app.route('/api/send-advanced-rules', methods=['POST'])
def api_send_advanced_rules():
    if 'logged_in' not in session:
        return jsonify({'error': 'غير مصرح'}), 401
    ...
    try:
        ...
        rules_title = request.form.get('rules_title')
        rules_list = json.loads(request.form.get('rules_list', '[]'))
        ...
        if image_file and image_file.filename:
            ...
            image_url = "https://via.placeholder.com/400x200/667eea/ffffff?text=Rules+Image"
        ...
        logger.error(f"Error in send_advanced_rules API: {e}")
...
if __name__ == "__main__":
    ...
    if connected:
        logger.info("✅ البوت المتقدم متصل وجاهز للعمل")
